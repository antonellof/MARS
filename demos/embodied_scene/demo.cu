...
//  embeddings; success = same-episode TEXT + AUDIO in top-K (BFS + bridges).
// ============================================================================
#include <cstdio>
#include <cstdlib>
#include <cstring>
...
int main(int argc, char** argv) {
    ...
    report.scenario        = "Kids + ball: IMAGE probes, same-episode TEXT+AUDIO in top-K";
    ...
    QueryContext ctx = create_query_context(graph.num_nodes, EMBEDDING_DIM, TOP_K);
    ...
    for (int32_t frame = 0; frame < NUM_FRAMES; ++frame) {
        ...
        if (episode_cross_modal_hit(results, corp.episode_ids[node],
                                    corp.episode_ids, TOP_K))
            ++hits;
    }

    report.add_metric("episode_cross_modal_hit_rate",
