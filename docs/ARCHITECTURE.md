...
![Episode-scoped scaling curve](../paper/figures/fig_episode_scoped_scaling.png)

*Same-hardware comparison on log–log axes. Episode-scoped MARS (green
diamond) is near-flat at ~200 µs across three decades of N while every
cosine-only baseline either crosses the 1 ms AV deadline (FAISS Flat at
N≥100K) or loses recall on this metric (CAGRA hit@15 = 0 at N≥100K).
Generated from `results/competitors_20260417/` by
[`scripts/generate_competitor_figures.py`](../scripts/generate_competitor_figures.py).*
...
*Wall-clock p99 (left, log scale) and `hit@15` (right, linear) for
six systems at N ∈ {10K, 100K, 1M}. MARS Episode-scoped (rightmost
green bar) is the only system simultaneously below 1 ms and at perfect
recall everywhere. Generated by
[`scripts/generate_competitor_figures.py`](../scripts/generate_competitor_figures.py)
...
![FP16 vs cuBLAS crossover](../paper/figures/fig_fp16_crossover.png)
