...
\item \textbf{Episode-scoped retrieval} delivers \textbf{197~$\mu$s
  $p99$ at $N{=}1$~M} on a synthetic embodied multimodal benchmark
  (10-node cross-modal episodes of one IMAGE, three AUDIO and six
  TEXT vectors; image query $\to$ same-episode text+audio; full
  setup in \S\ref{sec:competitor_bench}), while keeping perfect
  cross-modal hit@15. This is $33\times$ faster than the only other
  system that retains hit@15${=}1.0$ at that scale (FAISS-Flat-GPU,
  6.64~ms) and the only configuration that stays under the 1~ms AV
  deadline at $N{=}10^6$.
\item Cosine-only GPU ANN baselines (FAISS-IVF, cuVS CAGRA) lose
  hit@15 on this same benchmark as $N$ grows ($\le 0.4$ at
  $N{=}10^5$, $0$ at $N{=}10^6$). The dominant signal at scale is
  not ANN parameters but \emph{whether} the kernel sees episode
  membership.
...
We use this scene as the paper's running example and refer to it
throughout as the \emph{kids-ball benchmark}: a synthetic
multimodal corpus of 10-node cross-modal episodes (one IMAGE,
three AUDIO, six TEXT vectors per episode; within-episode cosine
$> 0.85$, across-episode $\sim$0.2--0.4 in a shared 768-D space)
queried with an IMAGE probe whose ground-truth answer is the
same-episode TEXT+AUDIO neighbours. The full generator and probe
contract are described in \S\ref{sec:competitor_bench}; we surface
the name here so every later headline number can be read against
this single, fixed contract.
...
\cdot D)$, which on the kids-ball benchmark just defined
...
\FOR{$\text{hop} = 0$ \TO $h-1$}
    \FOR{each node $v \in F_{\text{hop}}$ \textbf{(one warp per node)}}
        \FOR{each neighbor $u$ of $v$ \textbf{(stride-32 coalesced)}}
            \STATE prev $\leftarrow$ atomicCAS(visited[$u$], 0, 1)
            \IF{prev $= 0$}
...
\paragraph{atomicCAS contract used in lines~16--17.}
We rely on the standard CUDA \texttt{atomicCAS(addr, cmp, val)}
contract: the atomic returns the value previously stored at
\texttt{addr}, and the swap to \texttt{val} succeeds iff that previous
value equals \texttt{cmp}. The BFS expansion therefore writes to
\texttt{score[$u$]} and pushes $u$ onto the next frontier exactly when
the previous value of \texttt{visited[$u$]} was $0$ (unvisited),
which guarantees that each node's score is written by exactly one
warp regardless of multi-edge re-entry.

\paragraph{Score propagation parameters in line~18 of Algorithm~\ref{alg:pipeline}.} The two scalars
...
\paragraph{A note on the name.} The ``Neural'' in NSN is retained
for continuity with prior \mars{} releases and refers to its role
as a \emph{neural-embedding} index --- not to a learned graph
topology. The construction below is fully deterministic and contains
no trained weights, gradient-tuned objectives, or learned edge
selection; it is a Watts--Strogatz~\cite{wattsstrogatz} small-world
graph extended with deterministic cross-modal bridges.
...
Let $|\textnormal{ep}|$ be the size of the named episode, let $D$ be
the embedding dimension in bytes (e.g.\ $4D{=}3072$ for FP32 at
$D{=}768$), and let $B_{\textnormal{eff}}$ be the achieved HBM
bandwidth during cuBLAS \texttt{Sgemv}. The wall-clock cost of the
episode-scoped path is
\[
T_{\textnormal{ep}}(|\textnormal{ep}|) \;\approx\;
\frac{4 \cdot |\textnormal{ep}| \cdot D}{B_{\textnormal{eff}}}
\;+\; T_{\textnormal{topK}}(|\textnormal{ep}|) \;+\; T_{\textnormal{BFS,0}},
\]
with $T_{\textnormal{BFS,0}}$ the zero-hop BFS bookkeeping cost
($\sim$5\,$\mu$s of launch overhead, no neighbour expansion). The
global path costs
\[
T_{\textnormal{glo}}(N) \;\approx\;
\frac{4 \cdot N \cdot D}{B_{\textnormal{eff}}}
\;+\; T_{\textnormal{topK}}(N) \;+\; T_{\textnormal{BFS}}(K, h, \bar{d}).
\]
$T_{\textnormal{ep}} < T_{\textnormal{glo}}$ for any
$|\textnormal{ep}| \ll N$ once the bandwidth term dominates; on the
A100~SXM4 with $4D{=}3072$\,B, $|\textnormal{ep}|{=}10$, and
$B_{\textnormal{eff}}\!\approx\!1.2$\,TB/s, the inequality holds for
all $N \gtrsim 10^3$. Beyond the latency comparison, the
episode-scoped path \emph{eliminates by construction} all
...
benchmark with $|\textnormal{ep}|{=}10$ and $D{=}768$, the inequality
holds across the entire tested range, which is consistent with the
measured ${\sim}13\times$ speedup at $N{=}10^6$
...
configurations and \mars{}. ``Visibility miss'' = fraction of queries
whose target was inserted after the last index visibility refresh
(distinct from the deadline-miss rate reported for the demonstrators
in Table~\ref{tab:wall_clock}).}
\label{tab:streaming}
\begin{tabular}{llrr}
\toprule
\textbf{System} & \textbf{Insert mode} & \textbf{Visibility miss} & \textbf{Stall (ms)} \\
\midrule
...
spot-checked at 60--75\,\% on the A100~SXM4~80\,GB row at
$N{=}10^6$ via Nsight Systems --- a full per-stage Nsight Compute
breakdown across all rows is queued as part of the evaluation
hardening track (\S\ref{sec:future-work}). CUDA versions and driver
branches are the ones used at measurement time on the rented or
local machine.}
\label{tab:hwindex}
...
\subsection{Sustained 30-Second Benchmarks}
\label{sec:sustained}

Short-run benchmarks (10--50~seconds) may not capture effects that
emerge at longer durations: GPU boost-clock throttling, memory
allocator fragmentation, OS scheduler interference. We extend the
benchmarks to 15--30~seconds with larger corpora; longer (multi-minute
to multi-hour) sustained runs that would catch slow-onset thermal
drift are queued as part of the evaluation hardening track
...
\textbf{(1) \mars{} Episode-scoped is Pareto-optimal on the embodied
contract at every $N$.} It is $33\times$ faster than the only baseline
